...
export default function SupportChat() {
  ...
  const fetchMessages = async (customerId) => {
    ...
    try {
      const response = await api.get(`/support/conversations/${customerId}/messages`);
  ...
  useEffect(() => {
    ...
    const channel = echo.private(channelName)
      ...
      .listen(".support.message.sent", (data) => {
        ...
        const isFromCustomer = incomingMsg.sender_id === incomingMsg.customer_id;

        // If currently viewing this customer's chat room
        const isCurrentActive = selectedConvoRef.current && selectedConvoRef.current.customer.id === incomingMsg.customer_id;
  ...
  const handleSendMessage = async (e, customText = null) => {
    ...
    try {
      ...
      const response = await api.post(`/support/conversations/${customerId}/messages`, {
        message: textToSend,
      });
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col gap-6 h-[calc(100vh-140px)] animate-in fade-in duration-500">
        ...
        {/* Header bar */}
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex-shrink-0">
          <div>
            <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
              Tư vấn trực tuyến <Sparkles className="w-5 h-5 text-yellow-500 fill-yellow-500 animate-pulse" />
            </h1>
        ...
        <div className="flex-1 flex gap-6 overflow-hidden min-h-0">
          ...
          <div className="w-[350px] bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
            {/* Search */}
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-50/80">
              {loadingConvos ? (
                ...
                filteredConvos.map((convo) => {
                  const isSelected = selectedConvo && selectedConvo.customer.id === convo.customer.id;
                  const hasUnread = convo.unread_count > 0;
                  const isLastMsgAdmin = convo.last_message && convo.last_message.sender_id !== convo.customer.id;
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-baseline mb-0.5">
                          <h4 className={`text-sm font-bold truncate ${hasUnread ? "text-slate-900" : "text-slate-700"}`}>
                            {convo.customer.name}
                          </h4>
                        ...
                        <p className={`text-xs truncate ${hasUnread ? "text-indigo-600 font-extrabold" : "text-slate-400"}`}>
                          {isLastMsgAdmin && <span className="font-semibold text-slate-500 mr-1">Bạn:</span>}
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
            {selectedConvo ? (
              <>
                ...
                <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-slate-50/20">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-slate-800">{selectedConvo.customer.name}</h3>
                      <p className="text-[11px] text-slate-400 font-semibold">{selectedConvo.customer.email}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-slate-500 font-semibold">Đang kết nối</span>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/30 scroll-smooth">
                  {loadingMessages ? (
                    <div className="flex flex-col items-center justify-center h-full gap-2">
                      ...
                      <p className="text-xs text-slate-400 font-medium">Đang tải lịch sử tin nhắn...</p>
                    </div>
                  ) : messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center text-slate-400">
                      <MessageSquare className="w-8 h-8 text-slate-200 mb-2" />
                      <p className="text-xs font-semibold">Hãy gửi lời chào đầu tiên để hỗ trợ khách hàng!</p>
                    </div>
                  ) : (
                    messages.map((msg) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`flex items-center gap-1 mt-1 text-[9px] text-slate-400 font-medium ${isMe ? "mr-1" : "ml-10"}`}>
                ...
                <div className="px-4 py-2 border-t border-slate-50 bg-slate-50/10 flex items-center gap-2 overflow-x-auto whitespace-nowrap hide-scrollbar flex-shrink-0">
                  <div className="flex items-center gap-1.5 text-indigo-600 shrink-0 text-xs font-extrabold uppercase mr-1">
                    <Zap className="w-3.5 h-3.5 fill-indigo-100" /> Trả lời nhanh:
                  </div>
              ...
              <div className="flex flex-col items-center justify-center p-20 text-center text-slate-400 h-full">
                ...
                <h3 className="text-base font-bold text-slate-700 mb-1">Hộp thư hỗ trợ trực tuyến</h3>
                <p className="text-xs text-slate-400 max-w-[280px] leading-relaxed mx-auto font-medium">
                  Chọn một khách hàng trong danh sách bên trái để xem tin nhắn và tư vấn thời gian thực.
                </p>
              </div>
            )}
          </div>
