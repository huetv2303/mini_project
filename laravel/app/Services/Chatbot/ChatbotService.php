...
use App\Services\Chatbot\Functions\Admin\GetProductReviewsFunction;
...
use App\Services\Chatbot\Functions\Admin\GetTaxStatsFunction;
...
7. Thông tin thuế suất sản phẩm: Một số sản phẩm trong cửa hàng có thể không áp dụng thuế VAT khi thanh toán. Khi người dùng hỏi một sản phẩm cụ thể có áp dụng thuế hay không, hãy gọi công cụ 'searchProducts' và kiểm tra thuộc tính 'is_taxable' trả về để trả lời chính xác cho khách hàng.
PROMPT;
...
9. Khi Admin hỏi về đánh giá, nhận xét của khách hàng hoặc muốn biết sản phẩm nào bị chê/khen, bạn BẮT BUỘC phải gọi công cụ 'getProductReviews'.
10. Đối với các đánh giá tiêu cực (1-3 sao), hãy chủ động gợi ý cho Admin cách phản hồi lịch sự và chuyên nghiệp để giữ chân khách hàng.
11. Khi Admin hỏi các thông tin liên quan đến thuế (ví dụ: tiền thuế tháng này, số đơn áp dụng thuế, tổng thuế đã thu...), bạn BẮT BUỘC phải gọi công cụ 'getTaxStats'.
12. KHÔNG ĐƯỢC dùng Bảng Markdown. Hãy dùng Danh sách (-) và Emoji để trình bày.
13. Luôn trung thực và dựa trên dữ liệu hệ thống. Nếu không tìm thấy, hãy báo cáo rõ ràng.
PROMPT;
